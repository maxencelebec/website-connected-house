...
$req = $bdd->prepare('SELECT num_capteur, valeur FROM trame_courante AS t1 WHERE timestamp =
                     (SELECT MAX(timestamp) FROM trame_courante AS t2 WHERE t1.num_capteur = t2.num_capteur)
                      GROUP BY num_capteur');
$req->execute();
